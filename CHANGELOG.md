...
- The Graphs tab's **Ports** display could break with `Rrd::get_data_path(): Argument #2 ($port) must be of type int, string given`, leaving the correct caption but no graph ([#160](https://github.com/mbolli/nfsen-ng/issues/160)). `graph_ports` and `graph_sources` had no value in the rendered page, so Datastar invented them from the DOM the first time it applied a `data-bind` — for the Ports multi-select that means the option values as *strings*, and for the sources it meant whatever the Flows picker further down the page happened to hold. The server's own values only arrived with the first SSE push, so any filter change made before that posted the bogus ones back and made them stick. Both signals are now seeded from the server value in the markup, and the ports selection is coerced to `int` server-side before it reaches a datasource.
