...
- The MCP server can also be served over HTTP, at `/_mcp` on nfsen-ng's own port, with `NFSEN_MCP_HTTP=true`. Same ten tools, same limits, for an agent that does not live on the host. Sharing the port is deliberate: nfsen-ng has no authentication of its own and is protected by where it is deployed, so an endpoint on a second port would sit outside whatever guards the dashboard, while on this one it inherits that protection and needs no separate credentials. `NFSEN_MCP_HOSTS` lists the hostnames a client may address the server as, for the DNS-rebinding protection the specification asks for; empty means localhost only, and a request with any other `Host` gets `403`. With the flag off, `/_mcp` answers `404` like any path that does not exist.

- An optional [MCP](https://modelcontextprotocol.io) server exposes this installation's NetFlow data to an AI agent, read-only, for investigating traffic without writing nfdump filter expressions by hand. It is off by default and speaks stdio, so a client launches it as a subprocess (`php backend/mcp.php`) and there is no listening socket or credential to manage. Ten tools in two tiers, and every description states which: the cheap ones answer from the stored five-minute aggregates immediately, while `top_talkers`, `flow_matrix` and `list_flows` read capture files and cost time proportional to the window. `estimate_cost` prices a window before you commit to it. The server enforces its own limits rather than asking the model to behave: windows are clamped to `NFSEN_MAX_STATS_WINDOW`, row limits are capped, a per-call ceiling refuses a query that would read more than 16 GiB of captures, and filter expressions reach nfdump as a single escaped argument. Nothing in it writes — no rule creation, no import triggering, no settings changes — so the worst case is disclosure of flow data rather than control of the box. See [the MCP chapter](https://mbolli.github.io/nfsen-ng/features/mcp.html).

- **Backfill** on the Import panel imports capture files older than the newest sample already stored ([#171](https://github.com/mbolli/nfsen-ng/issues/171), reported by [@alexgill](https://github.com/alexgill)). A normal import resumes at that sample and never looks behind it, so an archive of `nfcapd` files collected before nfsen-ng was installed was skipped entirely. Backfill re-reads every capture file and writes each one to the slot it belongs to, deleting nothing, and it can be cancelled like any other import. It appears only on VictoriaMetrics, where a sample is addressed by its timestamp; RRD keeps the destructive **Force Rescan**, because RRDTool refuses an update at or before a file's last update and filling in history there means rebuilding the file. Datasources declare which of the two they need through a new `acceptsHistoricWrites()` on the datasource interface, so the panel stops offering a reset that would have done nothing. Note that VictoriaMetrics drops samples older than its retention window silently on ingest, so `--retentionPeriod` has to cover your oldest capture before backfilling.

- nfsen-ng now ships a favicon, so its tab is identifiable at a glance instead of showing the browser's blank default ([#170](https://github.com/mbolli/nfsen-ng/issues/170), suggested by [@gmt4](https://github.com/gmt4)). The mark is derived from the Unraid app icon — the same palette, simplified to a Sankey crop that still reads at 16px. Modern browsers get `favicon.svg`, Safari and bare `/favicon.ico` requests get a 16/32/48 `.ico`, and iOS home screens get a 180px `apple-touch-icon.png`.

- Graphs can now be plotted through an nfdump filter, and a new **Investigate** view puts that graph and the flow table on one screen ([#166](https://github.com/mbolli/nfsen-ng/issues/166), requested by [@gmt4](https://github.com/gmt4)). Investigating traffic meant switching between two tabs to answer one question — *when* did this host talk to that one, and *what* were the flows — because the graph could only show what the database had already aggregated. The datasources store flows/packets/bytes per 5-minute slot, so a filter can't be applied to them after the fact; the new **Filtered** mode goes back to the nfcapd files and runs one `nfdump -s proto` per time bin, which is what the per-port RRDs have always done with a filter hardcoded to `dst port N`. The number of nfdump runs is bounded by the requested resolution rather than by the window width, and the bytes read are the same bytes the Flows tab already reads for that range. Because it is expensive it never runs on its own: no live tick, no refresh on a filter keystroke, only an explicit **Apply filter**, with the result cached so re-rendering the page is free. **Stored** mode is the default and behaves exactly as before. The Investigate view reuses the existing Graphs and Flows panels rather than duplicating them, so the window, the source selection and the graph-zoom-to-date-range sync are shared automatically — drag a region on the graph and the flow query follows it. There is a single filter expression on screen: the flows panel hides its own filter box and sources select there and follows the graph's.

- The **Process data** buttons on Flows and Statistics, and **Apply filter** on the filtered graph, now show real progress instead of an indeterminate spinner, and their queries no longer block the request. A slow window used to be indistinguishable from a stuck one. The filtered graph reports *exactly* — it knows its bin count up front, so the bar tracks intervals scanned. Flows and Statistics run a single nfdump over the whole window and nfdump emits no progress of its own, so what is reported there is how much it has read (`/proc/<pid>/io`'s `rchar` against the total size of the capture files in range), shown as "Read 21 MiB of 44 MiB" and clearly marked an estimate. procfs is Linux-only; elsewhere the button falls back to the previous spinner and nothing else changes. **Kill** now also stops a filtered-graph build between bins — `SIGTERM` alone only ends the nfdump currently running — and keeps whatever intervals had already been scanned.
...
- `NFSEN_PORT_DIRECTION` decides which side of a flow a per-port graph counts, and answers [#173](https://github.com/mbolli/nfsen-ng/issues/173), reported by [@Ondo-Dulguun](https://github.com/Ondo-Dulguun). Their port graphs were empty while their per-source graphs worked, because their exporter reports one direction of each flow: port 443 appeared only as a *source* port, so the importer's `dst port 443` query matched nothing. Setting `NFSEN_PORT_DIRECTION=any` counts the port in either direction and fixes that. **The default stays `dst`**, which is what every release so far counted: widening it silently would step every existing port series upward against data already on disk under the old meaning. `src` is available too. Under `any` the importer counts only the rows for the port it is writing, because nfdump's either-direction statistic reports both ports of every matching flow and summing them all would roughly double each value. Separately, a configured port that saw no traffic during a run is now named once in the import log, so a flat graph says why rather than leaving you to guess whether collection is broken.

- The **Investigate** view is gone, replaced by the traffic panel on the Flows tab above. It was never what [#166](https://github.com/mbolli/nfsen-ng/issues/166) asked for: the request starts in Flows, where the filter already is, while Investigate was a third tab whose control panel was the union of the graph's and the flow table's, with nothing saying which control affected which half. Anyone whose browser had it selected is moved to Flows on their next load rather than landing on a blank page. Filtered graphs on the Graphs tab are unaffected: that is the part of #166 that stays.

- Concurrent nfdump queries no longer make **Kill** ambiguous, and the documented `NFSEN_NFDUMP_MAX_PROCESSES` limit is now enforced against the processes nfsen-ng actually started. It used to be enforced by counting every nfdump on the machine with `ps`/`pgrep`, which counted runs this app never started, raced between the count and the spawn, silently enforced nothing when neither tool was installed, and turned "busy" into an error rather than a short wait. A second query now waits for a free slot and reports plainly if none frees up. The running process is tracked per query instead of in one static, so Kill stops the query you asked about rather than whichever started last — which matters as soon as anything queries alongside the browser. A slot is taken per nfdump invocation, so a filtered-graph build running hundreds of them in sequence does not hold the ceiling for its whole duration. The Health panel's Max processes row now shows live usage, and its companion `ps`/`pgrep` availability check is gone, since nothing depends on those tools for this any more. `NFSEN_NFDUMP_MAX_PROCESSES` now defaults to `2` rather than `1`: the old check compared with `>` instead of `>=`, so a cap of one permitted two runs in practice, and the import daemon's nfdump runs take slots like any other, so a strict cap of one would make browsing queue behind an import.

- Port graphs ran empty while source and protocol graphs kept updating, on an install whose per-port RRDs existed and whose capture files held matching flows ([#173](https://github.com/mbolli/nfsen-ng/issues/173), reported with a root-cause analysis by [@Ondo-Dulguun](https://github.com/Ondo-Dulguun)). Only a bulk import ever wrote the per-source `source_port.rrd` files that the ports view reads: the importer the daemon keeps for ongoing inotify imports was constructed without port processing enabled, and `importFile()` gated its per-source pass on the wrong flag on top of that, so every capture file that arrived after the startup catch-up updated `source.rrd` and the all-sources `port.rrd` and nothing else. The per-port databases therefore stayed at whatever the last full import had left in them — all-NaN for a source added since, which is why the symptom showed up on a source rename — until the next manual import filled them again. The daemon's importer now enables both passes, `importFile()` gates them exactly as the bulk import does, and the all-sources query asks only for the sources whose capture has actually landed, which also clears the `stat() error ...: File not found!` lines the import log collected for files that were plainly there: the aggregate runs when the last source rotates its file, and names every source's copy of it, including the ones still a moment away.

- Relatedly, one flow of an unusual protocol could cost a capture file all of its port data. The per-port statistics are written under the protocol nfdump reports, but an RRD only has data sources for tcp, udp, icmp and other — the buckets of nfdump's own `-I` summary that `source.rrd` is filled from — so a GRE, ESP or SCTP flow on a monitored port made RRDTool reject the update with `unknown DS name 'flows_gre'`. That threw away every remaining port of that file and the per-source pass that runs after it, so on a network carrying such traffic the port graphs could stay empty for good. Those protocols now count towards `other` as they already do a level up, several of them summing rather than overwriting each other, and a port that fails for any other reason no longer takes the rest of the file down with it.

- A configured port with no traffic never got an RRD file, and selecting it then took the whole graph down with `rrd_xport failed ... No such file or directory` ([#172](https://github.com/mbolli/nfsen-ng/issues/172), reported with a root-cause analysis by [@Ondo-Dulguun](https://github.com/Ondo-Dulguun)). A force import always created a database for every source/port pair, but an incremental import only created one on a port's first write that returned data, so a quiet port stayed missing indefinitely. Incremental imports now create the missing port databases up front, the same layout force mode has always produced, and the graph treats a database that does not exist yet as an empty series instead of an error — so a port added to the configuration between imports draws nothing rather than breaking the other ports alongside it. No synthetic zero-value records are written: that would advance the import position past a capture file nfdump merely failed to read, recording a read failure as real zero traffic.

- The IP-info modal showed a bare IP heading over an empty table for every private address ([#168](https://github.com/mbolli/nfsen-ng/issues/168), reported with a patch by [@gmt4](https://github.com/gmt4)). A private IP gets no geolocation lookup at all, but the geo section rendered regardless, so the address appeared twice — once in the modal title, once over nothing. That section is now skipped when there is no geo data, and the Netbox table matches the styling of the geo one it sits above. Netbox rows stay curated rather than dumping the API object: `status`, `role`, `tenant` and `vrf` only read well through their `label`/`name`, and the raw response also carries `id`, `url` and timestamps nobody needs in a modal. `dns_name` and the assigned device/interface are shown now, which the previous list missed.

- A geolocation lookup that could not reach the service at all — no outbound internet, DNS failure, a timeout — returned nothing rather than an error, so the modal stayed silent about it. It now reports the failure like a provider-side error already did.

- The page no longer flashes light before switching to dark on every load ([#167](https://github.com/mbolli/nfsen-ng/issues/167), reported by [@not7cd](https://github.com/not7cd)). The theme was applied by Datastar, which is a module at the bottom of `<body>`, so the browser painted a complete light-themed frame first and corrected it a moment later — as the report puts it, enough to flash-bang someone in the middle of the night. It is now resolved by a small blocking script at the top of `<head>`, before the stylesheets, so the first frame is already the right theme.

- Relatedly, `NFSEN_DEFAULT_THEME=auto` stopped following the operating system after the first visit. `_darkMode` was persisted to localStorage on every change *including the initial seed*, so the OS preference read on a browser's first visit was saved and then restored unconditionally forever after — switch your desktop to light and nfsen-ng stayed dark until you toggled it by hand. What is stored now is the explicit choice you make with the toggle, not the resolved value, so 'auto' keeps meaning auto: it follows the OS across reloads, and while the tab is open. An explicit toggle still wins over both the OS and the deployment default. The old `nfsen-persist:_darkMode` key cannot be told apart from an auto-resolved value, so it is cleared on first load — if you had toggled the theme by hand, that one preference resets to your deployment default once.

- Follow-ups on the filtered graph, from reviewing it on screen rather than in code. The **Protocols** buttons did nothing in Filtered mode — every selection produced the same four series, and the selection was not even part of the cache key, so clicking one did not rebuild. They now behave as they do everywhere else: on the protocols display they choose which series exist, on the sources display they narrow the counter each source series is built from. **Display → Ports** was silently coerced to Protocols while the dropdown still offered it; the option is now disabled in Filtered mode, and switching to Filtered moves a ports view to protocols instead of leaving a dead selection. The window is now clamped by `NFSEN_MAX_STATS_WINDOW` like Statistics and Sankey already were — a filtered build re-reads every capture in the range, so an unbounded window was the same open-ended cost, just spent as minutes of nfdump. And instead of warning that "cost grows with the window", the panel now states it: *"Will read 288 capture files (2.2 MiB) in 404 nfdump runs."*

- The Investigate view showed the same state twice: a Sources select bound to the very same signal as the graph's, and a second identically-labelled nfdump filter box with a copy button between them. The flows panel now hides both there and follows the graph's filter, so there is one filter expression on screen instead of two and a copying step.

- The worker segfaulted on every request that made a curl call, on any PHP version, once libcurl reached 8.20.0 — which for nfsen-ng meant every page render on a VictoriaMetrics install and every alert webhook on any install. openswoole 26.2's native-curl hook cannot handle the way libcurl 8.20.0 changed its threaded resolver's use of `curl_multi_socket_action` ([curl#21558](https://github.com/curl/curl/issues/21558)): a hostname kills the worker whether or not it resolves, while a literal IP is fine. Bisected across releases — 8.19.0 and earlier are unaffected, 8.20.0 and later are not — so the hook is now switched off exactly when the running libcurl is 8.20.0 or newer. Every other hook stays, leaving stream-wrapper IO non-blocking; curl calls block the worker there until openswoole fixes it.
