...
- nfsen-ng could not serve a single page on PHP 8.5: the worker died with `signal=11` on every request. openswoole 26.2's native-curl hook segfaults on 8.5 — any `curl_exec()` inside a coroutine kills the worker, whether or not the host resolves — and the page render reaches one through `VictoriaMetrics::httpGet()`. Alert webhooks, the other curl caller, took the worker down the same way regardless of datasource. The native-curl hook is now dropped on 8.5 only; every other hook stays, so stream-wrapper IO remains non-blocking, and PHP 8.4 is untouched. Revert once openswoole fixes it upstream.
