...
- The Sankey tab failed outright on nfdump 1.7.5 with `NfDump error: Can not use print format fmt:%sa %da %dp %ibyt %ipkt %fl to aggregate flows` ([#159](https://github.com/mbolli/nfsen-ng/issues/159), reported in [#158](https://github.com/mbolli/nfsen-ng/issues/158)). That release refuses a custom `-o fmt:` format whenever `-A` aggregation is active, accepting only plain `csv`; user-selected formats with custom aggregation arrived in 1.7.6 ([nfdump #597](https://github.com/phaag/nfdump/issues/597)), which as an upstream enhancement will not reach Debian 13's `1.7.5-2+deb13u1`. 1.7.5 turns out to be a single-release island: 1.7.2-1.7.4 accept `fmt:` but answer `-o csv` with the wide legacy per-record schema that carries no `flows` column, so switching formats wholesale would have silently zeroed the per-link flow counts there. The Sankey now keeps `fmt:` everywhere except 1.7.5, where it reads nfdump's aggregated CSV instead and treats those column names (`srcAddr`, `dstAddr`, `dstPort`, `bytes`, `packets`, `flows`) as aliases of the `fmt:` tokens. The substitution is lossless, since 1.7.5 is also the release where that CSV gained `flows`. Verified against real 1.7.5 and 1.7.8 binaries: both produce byte-identical Sankey payloads. The `nfdump -V` probe moved out of `HealthChecker` into `Nfdump::version()`, cached per binary, so both callers share one probe.
- Changing the source selection on the Graphs tab did nothing on installations with more than one source: the graph kept showing every source, the clicked entry did not stay highlighted, and a `rrd_xport failed. opening '<rrd>/<profile>/.rrd': No such file or directory` banner appeared ([#160](https://github.com/mbolli/nfsen-ng/issues/160)). Datastar picks a `<select>`'s `data-bind` adapter once, when it applies the attribute, from `el.multiple`, and the Sources select only gained `multiple` afterwards, from `data-attr`. So it was always bound with the single-value adapter: writing the array signal to the DOM produced `String(["a","b"]) === "a,b"`, which matches no option and cleared the selection outright, and reading it back gave `+"" → NaN`, posted as `null`. The server saw no selection and fell back to all sources; when a round-trip left the signal an empty string instead, `(array) ''` reached the RRD path builder as one empty source name and rendered the bare `.rrd` filename from the error. Single-source installs were unaffected, since `String(["a"]) === "a"` still matches. The select now sets the signal from `selectedOptions` and applies it back (together with `multiple`, in that order) from one `data-effect`, so it is an array of real source names in every display mode, which is what the Flows/Statistics/Sankey selects bound to the same `graph_sources` signal already assumed. `graph_sources` is additionally seeded to match the configured default display, and sanitized server-side, so neither a stale nor a malformed selection can reach a datasource.
